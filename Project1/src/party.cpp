...
Candidate* Party::getPopularCandidate(){
  ...
  if (!candidates.empty()){
    for(int i = 0; i < candidates.size(); ++i){ //printing each the number of each candidates votes
      if (candidates[i]->getNumVotes() > max){ //updating the popular candidate if it has changed
        max = candidates[i]->getNumVotes();
        popularCandidate = candidates[i];
        index = i;
      }
      else if (candidates[i]->getNumVotes() == max){ //updating the popular candidate if it has changed
      //flip multiple times before deciding coin
        for (int j = 0; j < 100; ++j){
          c.flip();
        }
        //0 is heads and the existing popular candidate, tails in the next candidate with same vote count
        if (c.flip() == 1){ 
          popularCandidate = candidates[i];
          index = i;
        }
