...
void OPLDriver::run(std::string filename){
  ...
  //if there are more candidates than seats available, everyone should get a seat
  ...
  //Initialize parties
  std::string lastParty = " "; //this variable stores the most recent party: if the stream reads a new party, we create a new  party
  while(partyStream.good()){
    ...
    if(parties.size()>0){
      textResults.auditStream <<"Candidate " << newCandidate->getName() << " added to party " << parties.back().getName() <<  std::endl;
  ...
  while(std::getline(filein, line)){
    ...
    while(ballotStream.good()){
      ...
      if(tempCandidateVote.compare("1")==0){
        ...
        textResults.auditStream << candidates.at(candidateIndex)->getName() << " has recieved a vote and has " << candidates.at(candidateIndex)->getNumVotes() << " votes" << std::endl;
        break;
      }
      candidateIndex++;
    }
  ...
  for(int i=0; i<parties.size(); i++){
    ...
    textResults.auditStream << "Party " <<  parties.at(i).getName() << " has " << partyVotes << std::endl;
    ...
    textResults.auditStream << "Number of first allocation seats for party " << parties.at(i).getName() + ": " << partySeats << std::endl;
    //for each seat
  ...
  for(int i=0; i<(numSeats-seatsFilled); i++){
    ...
    for(int j=0; j<parties.size(); j++){
      if(parties.at(j).getNumCandidates()>0){
        ...
        textResults.auditStream << parties.at(j).getName() <<" has " << remainingVotes <<" remaining votes and " << parties.at(j).getNumCandidates() << " remaining candidates" << std::endl;
        ...
        if(remainingVotes > max){
          ...
          textResults.auditStream << "Current party " << parties.at(maxPartyIndex).getName() << ": HEADS(0)\nChallenging party " << parties.at(j).getName() << ": " << "TAILS(1)"<< std::endl;
          ...
          textResults.auditStream << parties.at(maxPartyIndex).getName() << " won, set to take a second allocation seat"<< std::endl;
        }
      }
    }
    ...
    //recalculate winner's party's number of votes
    parties.at(maxPartyIndex).calculateTotalVotes();
  }
  ...
  for(int i=0; i<parties.size(); i++){
    double percentage_of_vote = ((double)parties.at(i).getTotalVotes())/((double) numBallots)*100.0;
    double percentage_of_seats = ((double)parties.at(i).finalSeats)/((double) numBallots)*100.0;
    ...
    std::cout << std::left << std::setw(seatsWidth) << std::setfill(' ') << parties.at(i).finalSeats - parties.at(i).getTotalVotes()/quota;
    std::cout << std::left << std::setw(seatsWidth) << std::setfill(' ') << parties.at(i).finalSeats;
    std::cout << std::left << std::setw(percentageWidth) << std::setfill(' ') << std::setprecision(4) << percentage_of_vote << " / " << percentage_of_seats;
    std::cout<<std::endl;
  }
